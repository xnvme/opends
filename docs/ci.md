...
`build-ref` builds without CUDA, so only the ref backend and its tests
are compiled, then runs the ref test binaries against a pattern file in
the build directory. No block device is involved; the ref backend does
not require O_DIRECT.
